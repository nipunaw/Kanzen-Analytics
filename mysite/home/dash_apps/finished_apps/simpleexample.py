...
app3 = DjangoDash('inputgraph', external_stylesheets=external_stylesheets)
...
app3.layout = html.Div([
    html.H1('Custom Input Graph'),
    html.Button("Custom export", id="export_table", **{"data-dummy": ""}),
    dcc.Input(id='input1', type='text', value='Sailor Moon'),
    html.Button(id='submit-button', n_clicks=0, children='Submit'),
    dcc.Graph(id='slider-graph3', animate=True, style={"backgroundColor": "#1a2d46", 'color': '#ffffff'}),
    dcc.Slider(
        id='slider-updatemode2',
        marks={i: '{}'.format(i) for i in range(20)},
        max=20,
...
@app3.callback(
               Output('slider-graph3', 'figure'),
              [Input('submit-button','n_clicks'),
              State('input1','value')]
)
def display_value3(n_clicks, value):
    ...
    dic = {}
    for i in range(0,len(kw_grplist)):
        trendshow.build_payload(kw_grplist[i],timeframe = 'today 5-y',geo='')
        temp = trendshow.interest_over_time()
        if temp.empty:
            a=kw_grplist[i][0].split()[:4]
            b = [' '.join(kw_grplist[i][0].split()[:4])]
            trendshow.build_payload(b,timeframe = 'today 5-y',geo='')
            temp = trendshow.interest_over_time()
